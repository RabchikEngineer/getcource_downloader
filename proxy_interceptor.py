...
class Page:
    ...
    def add_video(self, file):
        ...
        if vid_id not in self.videos.keys():
            self.videos.update({vid_id: (self.vid_num, [(file, resolution)])})
            self.vid_num += 1
        elif not any([resolution == vid[1] for vid in self.videos[vid_id][1]]):
            self.videos[vid_id][1].append((file, resolution))
